	...
	public void execute(CommandSender source, String[] args) {
		...
		if(args.length == 0) {
			...
			if(!(source instanceof ProxiedPlayer)) {
				source.sendMessage(new TextComponent(BungeeConfig.ONLY_PLAYERS.color()
			...
			if (source.hasPermission(BungeeConfig.PING_PERMISSION.get(String.class))) {
				...
				if (ping < BungeeConfig.MEDIUM_MS.get(Integer.class)) {
					source.sendMessage(new TextComponent(BungeeConfig.PING.color()
							.replace("%prefix%", BungeeConfig.PREFIX.color())
			...
			if (ProxyServer.getInstance().getPlayer(args[0]) == null) {
				source.sendMessage(new TextComponent(BungeeConfig.NOT_ONLINE.color()
						.replace("%prefix%", BungeeConfig.PREFIX.color())
						.replace("%user%", (args[0]))));
				return;
			}
			...
			if (!(BungeeConfig.OTHERS_PING_OPTION.get(Boolean.class))) {
				source.sendMessage(new TextComponent(BungeeConfig.USAGE.color()
						.replace("%prefix%", BungeeConfig.PREFIX.color())));
				return;
			}
			...
			if (!(BungeeConfig.DYNAMIC_PING.get(Boolean.class))) {
				source.sendMessage(new TextComponent(BungeeConfig.OTHERS_PING.color()
						.replace("%prefix%", BungeeConfig.PREFIX.color())
						.replace("%user%", (args[0]))
