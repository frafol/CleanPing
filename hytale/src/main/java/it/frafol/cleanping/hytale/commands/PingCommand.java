	...
	protected CompletableFuture<Void> execute(@Nonnull CommandContext context) {
		...
		if (args.length == 0) {
			if (!(sender instanceof Player)) {
				sender.sendMessage(Message.raw(Placeholder.translate(HytaleMessages.ONLY_PLAYERS.get(String.class))
			...
			if (PermissionsModule.get().hasPermission(sender.getUuid(), HytaleConfig.PING_PERMISSION.get(String.class))) {
				if (!HytaleConfig.DYNAMIC_PING.get(Boolean.class)) {
					sender.sendMessage(Message.raw(Placeholder.translate(HytaleMessages.PING.get(String.class))
							.replace("%prefix%", Placeholder.translate(HytaleMessages.PREFIX.get(String.class)))
			...
			PlayerRef target = Universe.get().getPlayers().stream()
					.filter(p -> p.getUsername().equalsIgnoreCase(args[0]))
					.findFirst()
					.orElse(null);

			if (target == null) {
				sender.sendMessage(Message.raw(Placeholder.translate(HytaleMessages.NOT_ONLINE.get(String.class))
						.replace("%prefix%", Placeholder.translate(HytaleMessages.PREFIX.get(String.class)))
						.replace("%user%", args[0])));
				return CompletableFuture.completedFuture(null);
			}
			...
			if (!HytaleConfig.OTHERS_PING_OPTION.get(Boolean.class)) {
				sender.sendMessage(Message.raw(Placeholder.translate(HytaleMessages.USAGE.get(String.class))
						.replace("%prefix%", Placeholder.translate(HytaleMessages.PREFIX.get(String.class)))));
				return CompletableFuture.completedFuture(null);
			}
			...
			if (!HytaleConfig.DYNAMIC_PING.get(Boolean.class)) {
				sender.sendMessage(Message.raw(Placeholder.translate(HytaleMessages.OTHERS_PING.get(String.class))
						.replace("%prefix%", Placeholder.translate(HytaleMessages.PREFIX.get(String.class)))
						.replace("%user%", args[0])
						.replace("%ping%", String.valueOf(ping))));
				return CompletableFuture.completedFuture(null);
			...
			sender.sendMessage(Message.raw(Placeholder.translate(HytaleMessages.PING_DIFFERENCE.get(String.class))
					.replace("%prefix%", Placeholder.translate(HytaleMessages.PREFIX.get(String.class)))
					.replace("%arg1%", args[0])
					.replace("%arg2%", args[1])
